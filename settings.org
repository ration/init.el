...
(use-package yasnippet
  ...
  (setq yas-snippet-dirs '("/home/lahtela/.emacs.d/elpa/yasnippet-snippets-20200802.1658/snippets" "~/.emacs.d/snippets")))
...
(defun ts-goto-test-or-source ()
  ...
  (let* ((file (buffer-file-name))
         (test (string-match "\\(.*?\\)\\(\.test\\)?\\(\.ts\\)$" file)))
    (message "%s %s" (match-string 2 file) file)
...
(use-package find-trace-paths
  :quelpa (find-trace-paths
           :fetcher github :repo "ration/find-trace-paths"))
